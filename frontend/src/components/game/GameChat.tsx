...
export const GameChat: React.FC<GameChatProps> = ({
  ...
  const containerClasses = [
    'flex min-h-0 h-full max-h-full flex-1 flex-col overflow-hidden rounded-xl border border-border/70 shadow-lg motion-safe:transition-all motion-safe:duration-200',
    isExpanded
  ...
  return (
    <Card className={containerClasses}>
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col gap-4">
        <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden">
          ...
          {pinnedMessages.length > 0 && (
            <div className="space-y-2">
              <div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">공유</div>
          ...
          <div
            ...
            tabIndex={0}
            className={`relative flex min-h-0 flex-1 overflow-y-auto rounded-2xl border border-border/60 bg-muted/40 px-4 py-4 shadow-inner transition-colors scroll-smooth chat-scroll-area ${isExpanded ? 'max-h-[calc(100vh-8rem)]' : 'sm:min-h-[18rem]'}`}
          >
            {!hasMessages ? (
        ...
        <form className="flex flex-col gap-2" onSubmit={handleComposerSubmit} noValidate>
          <div
          ...
          <div className="min-h-[1.5rem]" aria-live="assertive">
            {sendError && (
