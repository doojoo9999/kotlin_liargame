...
const OptimizedEnhancedChatSystem = ({
  ...
  useEffect(() => {
    if (convertedMessages.length > 0) {
      ...
      if (!lastOptimizedId || 
          convertedMessages[convertedMessages.length - 1].id !== lastOptimizedId) {
        
        const lastOptimizedIndex = lastOptimizedId ? 
          convertedMessages.findIndex(msg => msg.id === lastOptimizedId) : -1
        
        const newMessages = lastOptimizedIndex >= 0 ? 
          convertedMessages.slice(lastOptimizedIndex + 1) : convertedMessages
        
        if (newMessages.length > 0) {
          addMessages(newMessages)
        }
      }
    } else if (convertedMessages.length === 0 && optimizedMessages.length > 0) {
      clearMessages()
    }
  }, [convertedMessages, optimizedMessages, addMessages, clearMessages])
