...
describe('gameStoreV2 state machine', () => {
  ...
  it('initializes and starts SPEECH with per-player rotation', () => {
    ...
    s.startGame()
    expect(useGameStoreV2.getState().phase).toBe('SPEECH')
    ...
    useGameStoreV2.getState().setTimeRemaining(1)
    s.tick()
    expect(useGameStoreV2.getState().phase).toBe('VOTING_FOR_LIAR')
  })

  it('cycles phases and advances round after GAME_OVER', () => {
    const s = useGameStoreV2.getState()
    s.initialize('g1', [{ id: 'p1', nickname: 'Player1' }], '과일', 2)
    s.startPhase('GAME_OVER')
    expect(useGameStoreV2.getState().phase).toBe('GAME_OVER')
