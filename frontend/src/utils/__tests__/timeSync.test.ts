...
describe('scoreCalculations', () => {
  it('LIAR_ELIMINATED awards +3 to correct voters', () => {
    const res = calculateScoreChanges('LIAR_ELIMINATED', { liars, citizens, correctVoters: ['C1'], incorrectVoters: [] })
    expect(res).toEqual([{ playerId: 'C1', delta: 3 }])
  })

  it('INNOCENT_ELIMINATED: liars +4, eliminate voters -1, savers +1', () => {
    const res = calculateScoreChanges('INNOCENT_ELIMINATED', { liars, citizens, correctVoters: ['C2'], incorrectVoters: ['C1'] })
  ...
  it('LIAR_SURVIVED: liar +6', () => {
    const res = calculateScoreChanges('LIAR_SURVIVED', { liars, citizens, correctVoters: [], incorrectVoters: [] })
    expect(res).toContainEqual({ playerId: 'L1', delta: 6 })
  })

  it('LIAR_GUESSED_TOPIC: guesser +3', () => {
    const res = calculateScoreChanges('LIAR_GUESSED_TOPIC', { liars, citizens, guesser: 'L1' })
