...
async function testGameFlow() {
  ...
  try {
    ...
    console.log('\n1. Creating a game...');
    const createResponse = await fetch('http://localhost:20021/api/v1/game/create', {
      method: 'POST',
      headers: {
    ...
    const stateResponse = await fetch(`http://localhost:20021/api/v1/game/${gameNumber}`, {
      method: 'GET',
      credentials: 'include',
    });
    ...
    const joinResponse = await fetch('http://localhost:20021/api/v1/game/join', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
