...
class TestSuiteManager {
  ...
  async waitForServices(timeout = 30000) {
    ...
    while (Date.now() - startTime < timeout) {
      try {
        execSync('pg_isready -h 218.150.3.77 -p 5433', { stdio: 'pipe' });
...
module.exports = defineConfig({
  ...
  use: {
    baseURL: process.env.BASE_URL || 'http://218.150.3.77:3000',
...
jobs:
  ...
  integration-tests:
    ...
    - name: Run database migrations
      ...
      env:
        DATABASE_URL: postgresql://postgres:postgres@218.150.3.77:5432/test_db
    
    - name: Run integration tests
      run: npm run test:integration
      env:
        DATABASE_URL: postgresql://postgres:postgres@218.150.3.77:5432/test_db
        REDIS_URL: redis://218.150.3.77:6379
