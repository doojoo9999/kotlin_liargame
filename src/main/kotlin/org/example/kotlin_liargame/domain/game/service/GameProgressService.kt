...
class GameProgressService(
    ...
    @field:Lazy private val chatService: org.example.kotlin_liargame.domain.chat.service.ChatService, // 남은 기능(삭제/히스토리 용)
    ...
    fun startGame(session: HttpSession): GameStateResponse {
        ...
        try {
            ...
            chatSystemMessenger.sendSystemMessage(savedGame, "🎮 게임이 시작되었습니다!")
            chatSystemMessenger.sendSystemMessage(savedGame, "📝 각자 받은 주제에 대한 힌트를 차례대로 말해주세요.")

            when (savedGame.gameMode) {
                GameMode.LIARS_KNOW -> {
                    chatSystemMessenger.sendSystemMessage(savedGame, "🤫 라이어는 자신이 라이어임을 알고 있습니다. 다른 사람들의 힌트를 잘 들어보세요!")
                }
                GameMode.LIARS_DIFFERENT_WORD -> {
                    chatSystemMessenger.sendSystemMessage(savedGame, "🎭 라이어는 다른 주제의 단어를 받았습니다.")
                }
            }

            chatSystemMessenger.sendSystemMessage(savedGame, "⏰ 각 플레이어는 ${gameProperties.turnTimeoutSeconds}초 안에 힌트를 말해야 합니다.")
    ...
    fun forceNextTurn(gameId: Long) {
        ...
        game.currentPlayerId?.let { currentUserId ->
            ...
            if (currentPlayer != null && currentPlayer.state == PlayerState.WAITING_FOR_HINT) {
                ...
                try {
                    chatSystemMessenger.sendSystemMessage(game, "⏰ ${currentPlayer.nickname}님의 시간이 초과되어 다음 차례로 넘어갑니다.")
                } catch (e: Exception) {
