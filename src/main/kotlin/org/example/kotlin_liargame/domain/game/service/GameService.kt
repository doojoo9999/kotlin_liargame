...
class GameService(
    ...
    private fun validateExistingOwner(ownerName: String) {
        if (gameRepository.findBygOwner(ownerName) != null) {
            throw RuntimeException("?¥Î? ÏßÑÌñâÏ§ëÏù∏ Í≤åÏûÑ???åÏú†?òÍ≥† ?àÏäµ?àÎã§.")
        }
    }
    ...
    private fun findNextAvailableRoomNumber(): Int {
        ...
        throw RuntimeException("Î™®Îì† Î∞?Î≤àÌò∏(1-999)Í∞Ä Î™®Îëê ?¨Ïö©Ï§ëÏûÖ?àÎã§. ?òÏ§ë???§Ïãú ?úÎèÑ?¥Ï£º?∏Ïöî.")
    }
    ...
    fun startGame(req: StartGameRequest): GameStateResponse {
        ...
        val game = gameRepository.findBygOwner(nickname)
            ?: throw RuntimeException("Í≤åÏûÑ??Ï∞æÏùÑ ???ÜÏäµ?àÎã§. Î®ºÏ? Í≤åÏûÑÎ∞©ÏùÑ ?ùÏÑ±?¥Ï£º?∏Ïöî.")

        if (game.gState != GameState.WAITING) {
            throw RuntimeException("Í≤åÏûÑ???¥Î? ÏßÑÌñâ Ï§ëÏù¥Í±∞ÎÇò Ï¢ÖÎ£å?òÏóà?µÎãà??")
        }

        val players = playerRepository.findByGame(game)
        if (!game.canStart(players.size)) {
            throw RuntimeException("Í≤åÏûÑ???úÏûë?òÍ∏∞ ?ÑÌïú ?åÎ†à?¥Ïñ¥Í∞Ä Ï∂©Î∂Ñ?òÏ? ?äÏäµ?àÎã§. (ÏµúÏÜå 3Î™? ÏµúÎ? 15Î™?")
        }
    ...
    private fun createTestSubjects(): List<SubjectEntity> {
        ...
        val animalWords = listOf("?¨Ïûê", "?∏Îûë??, "ÏΩîÎÅºÎ¶?, "Í∏∞Î¶∞")
        ...
        val fruitWords = listOf("?¨Í≥º", "Î∞îÎÇò??, "?§Î†åÏßÄ", "?¨ÎèÑ")
