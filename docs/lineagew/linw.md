...
실제 운영 시 콘솔 메뉴 기준 프로세스:

1. **보스 등록** - 신규 보스와 난이도 메모를 정리한다.
2. **보스킬 기록** - 처치 시점, 참여 혈원, 획득 아이템을 남긴다.
3. **정산** - 아이템 판매 내역을 입력하고 분배 엔진으로 확정한다.
4. **분배(신규 탭)** - 혈원별 대기 중인 Payout 금액을 확인하고, 지급 완료 시 상태를 PAID로 전환한다.
5. **분배 확인(신규 탭)** - 혈원별로 어떤 보스/아이템에서 얼마를 받았는지 기간/상태별로 조회한다.
