...
async function testLoginPage() {
    console.log('[DEBUG_LOG] Checking if development server is running on 119.201.51.128:5173...');
    
    const isServerRunning = await checkServer();
    
    if (!isServerRunning) {
        console.log('[DEBUG_LOG] Development server not running. Please start it with: npm run dev');
        console.log('[DEBUG_LOG] Then access http://119.201.51.128:5173/login to verify the fix');
        return;
    }
    
    console.log('[DEBUG_LOG] Development server is running!');
    console.log('[DEBUG_LOG] Please manually test:');
    console.log('[DEBUG_LOG] 1. Open browser and navigate to: http://119.201.51.128:5173/login');
